...
func doCallgraph(dir, gopath, algo, format string, tests bool, args []string) error {
	...
	if err := callgraph.GraphVisitEdges(cg, func(edge *callgraph.Edge) error {
		...
		if strings.Contains(buf.String(), "tls.Config") {
...
func (e *Edge) Description() string {
	return e.edge.Description()
}

func (e *Edge) IsSuspected() bool {
	f := e.Callee
	if f != nil && f.Pkg != nil && f.Pkg.Pkg != nil {
		p := f.Pkg.Pkg
		functionName := f.Name()

		// find excluded packages and functions
		for _, x := range excludedFunctions {
			if x.pkg == p.Path() && (x.name == functionName || x.name == "*") {
				return true
			}
		}
...
func (e *Edge) IsInspected() bool {
	...
	if f != nil && f.Pkg != nil && f.Pkg.Pkg != nil {
		...
		// find inspected packages and functions
		for _, x := range inspectdFunctions {
			if x.pkg == p.Path() && (x.name == functionName || x.name == "*") {
...
func (e *Edge) PrintSuspected() {
	printFileLine(e.Filename(), e.Line())
}

func printFileLine(fileName string, line int) {
...
var excludedFunctions = []ExFunction{
	{"crypto/md5", "*"},
	{"crypto/md4", "New"},
	{"crypto/sha1", "*"},
}

var inspectdFunctions = []ExFunction{
	{"crypto/tls", "Server"},
	{"crypto/tls", "Client"},
	{"crypto/sha1", "*"},
}
